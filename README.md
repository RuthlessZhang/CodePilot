...
Remote endpoints must use HTTPS; plain HTTP is accepted only for `localhost`, `127.0.0.1`, and `::1`. Redirects are rejected. CodePilot uses the official MCP TypeScript SDK v2 and automatically negotiates between modern MCP 2026-07-28 servers and legacy 2025 servers. Streamable HTTP supports JSON and SSE responses, server event streams, session IDs, cancellation, reconnection, environment-backed Bearer authentication, and best-effort session deletion.

Servers that advertise tool-list changes are subscribed automatically. A validated update atomically replaces that server's tools in the live `ToolRegistry`, so the current Agent can use newly added tools without restarting. Updates that exceed CodePilot's limits or collide with another tool are rejected while the last valid tool set remains active. Interactive OAuth is intentionally deferred until CodePilot can persist access and refresh tokens in a native secure credential store; Bearer tokens continue to be read from environment variables on every request.
